...
def convnet(network, weights_path=None, heatmap=False,width=None,height=None,
            nb_classes=1000):
    """
    ...
    * For VGG16 and VGG19, the data are of shape (224,224), and the colors in the BGR order
    ...
    convnet = convnet_init(weights_path, heatmap=False, nb_classes=nb_classes)

    if not heatmap:
        return convnet
    else:
        convnet_heatmap = convnet_init(heatmap=True,nb_classes=nb_classes,width=width,height=height)

        for layer in convnet_heatmap.layers:
            if layer.name.startswith("conv"):
                orig_layer = convnet.get_layer(layer.name)
                layer.set_weights(orig_layer.get_weights())
            elif layer.name.startswith("dense"):    # weight reshape
                orig_layer = convnet.get_layer(layer.name)
...
def VGG_16(weights_path=None, heatmap=False, nb_classes=1000, width = None, height = None):
    model = Sequential()
    if heatmap:
        model.add(ZeroPadding2D((1,1),input_shape=(3,height,width)))  # because we can detect small region on big image
    else:
    ...
    if heatmap:
        ...
        model.add(Convolution2D(nb_classes,1,1,name="dense_3"))
...
def VGG_19(weights_path=None,heatmap=False,nb_classes=1000,width = None, height = None):
    model = Sequential()
...
def AlexNet(weights_path=None, heatmap=False, nb_classes=1000,width = None, height = None):
    if heatmap:
    ...
    if heatmap:
        ...
        dense_3 = Convolution2D(nb_classes, 1,1,name="dense_3")(dense_2)
...
# process all the iamges in the folder
...
if __name__ == "__main__":
    ...
    im = preprocess_image_batch(['/home/yang/Desktop/CVPR2016/MyCode/Keras_Code/JPGexamples/dog.jpg'], color_mode="rgb")

    # Test pretrained model
    sgd = SGD(lr=0.1, decay=1e-6, momentum=0.9, nesterov=True)
    model = convnet('alexnet',weights_path="/home/yang/Desktop/CVPR2016/MyCode/Keras_Code/weights/alexnet_weights.h5", heatmap=False)
    model.compile(optimizer=sgd, loss='mse')


    out = model.predict(im)
    heatmap = out[0,ids,:,:].sum(axis=0)
    plt.imsave("/home/yang/Desktop/CVPR2016/MyCode/Keras_Code/JPGexamples/heatmap_doggy.png",heatmap)
